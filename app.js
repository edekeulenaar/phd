...
// Cache key for the downloadable thesis.pdf. build_site_data.py hashes the
// file and puts the result in toc.json, so it changes whenever the PDF does.
// The constant below is only a fallback for an old toc.json; it must never be
// what readers actually get, or they keep the copy they downloaded months ago.
const PDF_VER_FALLBACK = "20260828b";
const pdfVer = () => (typeof TOC !== "undefined" && TOC?.pdfVersion) || PDF_VER_FALLBACK;
...
async function renderTermsVenn() {
  ...
  const N = CATS.length;
  const W = 1500, H = 1500, cx = W / 2, cy = H / 2;
  const R = 335, D = 240;     // circle radius / centre distance — D<R ⇒ common core
  const centres = CATS.map((c, i) => {
    const a = -Math.PI / 2 + (2 * Math.PI * i) / N;
    return [cx + D * Math.cos(a), cy + D * Math.sin(a), a];
  });
  const inside = (x, y, i) =>
    (x - centres[i][0]) ** 2 + (y - centres[i][1]) ** 2 <= R * R;
  const membership = (x, y) => CATS.map((_, i) => i).filter(i => inside(x, y, i));

  // Find the radial band along `angle` where membership equals exactly
  // `want` (a sorted index array); return its midpoint + length, or null.
  function bandAlong(angle, want) {
    const key = want.join(",");
    let r0 = null, r1 = null;
    for (let r = 0; r <= D + R; r += 4) {
      const x = cx + r * Math.cos(angle), y = cy + r * Math.sin(angle);
      const m = membership(x, y).join(",");
      if (m === key) { if (r0 === null) r0 = r; r1 = r; }
      else if (r0 !== null) break;     // band ended
    }
    if (r0 === null) return null;
    const mid = (r0 + r1) / 2;
    return { x: cx + mid * Math.cos(angle), y: cy + mid * Math.sin(angle),
             len: r1 - r0 };
  }
  ...
  function render() {
    ...
    CATS.forEach((c, i) => {
      svg.append("circle").attr("class", "vn-circle").attr("data-cat", c)
        .attr("cx", centres[i][0]).attr("cy", centres[i][1]).attr("r", R)
        .attr("fill", color(c)).attr("fill-opacity", 0.15)
        .attr("stroke", color(c)).attr("stroke-width", 1.8);
    });
    ...
    function block(g, x, y, name, nameFill, terms, lh, cls, max) {
      ...
      g.selectAll(null).data(ts).join("text")
        .attr("class", cls)
        .attr("x", x).attr("y", (d, j) => yy + j * lh)
        .attr("text-anchor", "middle")
        .text(d => d.Keyword)
        .append("title").text(d => `${d.Keyword} · ${fmtN(+d.Count)} mentions`);
    }

    // Exclusive regions — along each circle's own angle.
    CATS.forEach((c, i) => {
    ...
    CATS.forEach((c, i) => {
      const j = (i + 1) % N;
      const terms = pairOf(c, CATS[j]);
      if (!terms.length) return;
      let am = (centres[i][2] + centres[j][2]) / 2;
      if (i === N - 1) am += Math.PI;            // wrap-around midpoint
...
async function renderChapter(slug) {
  ...
  if (_pendingJump) {                       // arrived here from a cross-chapter comment click
    ...
    setTimeout(() => scrollToCommentMark(id, slug), 150);
...
// each labelled with the chapter it lives on, clickable, with a delete button.
function cmtChapterLabel(slug) {
  const e = tocEntry(slug);
  if (e) return e.title;
  if (slug === "home") return "Cover";
  if (slug === "references") return "References";
  return slug;                          // stale/unknown slug — show raw so it's diagnosable
}
function cmtRailRow(c, slug) {
  const stale = !tocEntry(slug) && slug !== "home" && slug !== "references";
  const ch = `<span class="cmt-rail-ch${stale ? " stale" : ""}">${escapeHtml(cmtChapterLabel(slug))}${stale ? " (chapter no longer exists)" : ""}</span>`;
  const who = `<span class="cmt-rail-who">${escapeHtml(c.name || "Anon")}</span>`;
  const body = `${escapeHtml((c.body || "").slice(0, 60))}${(c.body || "").length > 60 ? "…" : ""}`;
  return `<li class="cmt-rail-item" data-jump="${c.id}" data-chapter="${slug}" tabindex="0" role="button">` +
    `<button type="button" class="cmt-rail-del" data-cmt-del="${c.id}" data-cmt-del-ch="${slug}" title="Delete this comment" aria-label="Delete comment">×</button>` +
    `${ch}${who}: ${body}</li>`;
}
...
function scrollToCommentMark(id, slug) {
  ...
  window.scrollTo({ top: Math.max(0, y), behavior: "smooth" });   // belt-and-braces
...
// Click a rail comment → take the reader to its passage, navigating chapters if needed.
function jumpToComment(id, chapter) {
  ...
  if (target !== routeSlug()) {         // comment lives on another chapter
    _pendingJump = id;
    location.hash = `#/${target}`;      // route() → renderChapter → finishes the jump
    return;
  }
  scrollToCommentMark(id, target);
}
document.addEventListener("click", e => {
